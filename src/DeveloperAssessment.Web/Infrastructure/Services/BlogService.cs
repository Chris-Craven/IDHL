using DeveloperAssessment.Web.Models;
using Newtonsoft.Json;
using DeveloperAssessment.Web.Models;
using Newtonsoft.Json.Linq;

namespace DeveloperAssessment.Web.Infrastructure.Services;

public class BlogService : IBlogService
{
    public const string jsonFilepath = "./Assets/Blog-Posts.json";

    public BlogPost GetBlogPostById(int id)
    {
        var blogPostsResponse = GetJsonObject();

        var blogPost = (blogPostsResponse?.BlogPosts).FirstOrDefault(x => x.Id == id) ?? new BlogPost();

        switch (id)
        {
            case 1:
                blogPost.Image =
                    "https://images.freeimages.com/image/previews/a1a/study-kit-flat-png-design-5690859.png";
                break;
            case 2:
                blogPost.Image =
                    "data:image/png;base64,";
                break;
            case 3:
                blogPost.Image =
                    "https://growcreate.co.uk/media/mhhj4g3t/kentico-partner-logo.svg?rmode=min&width=1280&quality=90&v=1d8fb4081d617c0";
                break;
            default:
                blogPost.Image = "https://www.freeimages.com/clipart/study-kit-flat-png-design-5690859";
                break;
        }

        return blogPost;
    }

    public List<BlogPost> GetAllBlogPosts()
    {
        var blogPostsResponse = GetJsonObject();
        return blogPostsResponse?.BlogPosts.ToList() ?? new List<BlogPost>();
    }

    public void AddComment(CreateCommentViewModel commentViewModel)
    {
        var blogPostsResponse = GetJsonObject();

        var blogPost = blogPostsResponse?.BlogPosts.FirstOrDefault(x => x.Id == commentViewModel.BlogId);

        //There will definitely be a better way of doing this, but this way seems to work ok.
        if (blogPost != null)
        {
            var newComment = new BlogPostComment()
            {
                Id = Guid.NewGuid(),
                Name = commentViewModel.Name,
                Date = DateTime.Now,
                EmailAddress = commentViewModel.EmailAddress,
                Message = commentViewModel.Message
            };

            try
            {
                var filePath = HandleFile(newComment.Id, commentViewModel.File);
                newComment.ImageUrl = filePath;
            }
            catch (Exception e)
            {
                //Would handle if I had more time.
            }

            if (blogPost.Comments == null)
            {
                blogPost.Comments = new List<BlogPostComment>();
                var preUpdatedBlogFile = JsonConvert.SerializeObject(blogPostsResponse, Formatting.Indented);
                File.WriteAllText(jsonFilepath, preUpdatedBlogFile);
            }

            blogPost.Comments.Add(newComment);

            var updatedBlogFile = JsonConvert.SerializeObject(blogPostsResponse, Formatting.Indented);
            File.WriteAllText(jsonFilepath, updatedBlogFile);
        }
    }

    private BlogPostsResponse GetJsonObject()
    {
        using StreamReader reader = new(jsonFilepath);
        var json = reader.ReadToEnd();
        return JsonConvert.DeserializeObject<BlogPostsResponse>(json);
    }

    public void AddReply(CreateReplyViewModel replyViewModel)
    {
        var blogPostsResponse = GetJsonObject();

        var blogPost = blogPostsResponse?.BlogPosts
            ?.Where(x => x.Comments != null && x.Comments.Any(c => c.Id == replyViewModel.CommentId)).FirstOrDefault();
        var comment = blogPost.Comments.Where(x => x.Id == replyViewModel.CommentId).FirstOrDefault();

        //There will definitely be a better way of doing this, but this way seems to work ok.
        if (blogPost != null)
        {
            var newReply = new BlogPostReply()
            {
                BlogPostCommentId = replyViewModel.CommentId,
                Id = Guid.NewGuid(),
                Name = replyViewModel.Name,
                Date = DateTime.Now,
                EmailAddress = replyViewModel.EmailAddress,
                Message = replyViewModel.Message
            };

            if (comment.BlogPostReplies == null)
            {
                comment.BlogPostReplies = new List<BlogPostReply>();
                var preUpdatedBlogFile = JsonConvert.SerializeObject(blogPostsResponse, Formatting.Indented);
                File.WriteAllText(jsonFilepath, preUpdatedBlogFile);
            }

            comment.BlogPostReplies.Add(newReply);

            var updatedBlogFile = JsonConvert.SerializeObject(blogPostsResponse, Formatting.Indented);
            File.WriteAllText(jsonFilepath, updatedBlogFile);
        }
    }

    private string HandleFile(Guid commentId, IFormFile file)
    {
        var fileName = DateTime.Now.ToString("ddMMyyyyHHmmss") + "-" + file.FileName;

        var filePath = Path.Combine("wwwroot/UploadedFiles", fileName);

        using (var stream = new FileStream(filePath, FileMode.Create))
        {
            file.CopyTo(stream);
        }

        return fileName;
    }
}